...
describe('User Endpoints', () => {
  describe('GET /users/profile', () => {
    ...
    test('should return the correct user profile', async () => {
      ...
      jest.spyOn(require('../src/controllers/userController'), 'getProfile').mockImplementation((req, res) => {
  ...
  describe('PUT /users/profile', () => {
    test('should require authentication', async () => {
      const res = await request(app).put('/users/profile').send({ name: 'NewName' });
      expect(res.statusCode).toBe(401);
    });
    ...
    test('should handle server errors', async () => {
      ...
      jest.spyOn(require('../src/controllers/userController'), 'updateProfile').mockImplementation((req, res) => {
        throw new Error('Server Error');
      });
